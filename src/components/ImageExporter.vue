  ...
  try {
    ...
    for (const crop of image.crops) {
      ...
      const compressedFile = await imageCompression(
        new File(
          [blob],
          `${image.originalFile.name}-${crop.platform}-${crop.type}.${format.value}`,
          {
    ...
    toast.success('Fotografie úspěšně uloženy');
  } catch (error) {
...
<template>
  ...
  >
    <div class="flex space-x-4 items-center">
      <div class="space-y-2">
        <label class="block text-sm font-medium">Formát: </label>
      ...
      <div class="space-y-2">
        <label class="block text-sm font-medium">Kvalita: </label>
        <input
          type="range"
          v-model="quality"
          min="0.1"
    ...
    >
      {{ processing ? 'Zpracovávám...' : 'Uložit upravené fotky' }}
    </button>
  </div>
</template>
